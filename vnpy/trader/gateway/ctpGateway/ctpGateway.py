...
class CtpMdApi(MdApi):
    ...
    def __init__(self, gateway):
        ...
        self.defaultSubscribSymbols()         #默认订阅
    ...
    def defaultSubscribSymbols(self):
        subscribeDefault = json.load(open('./subscribeDefault.json', 'r'))
        for subscribe in subscribeDefault:
            if subscribe['symbol'] != u'':
                # 订阅合约
                req = VtSubscribeReq()
                req.symbol = subscribe['symbol']
                self.subscribedSymbols.add(req)
...
class CtpTdApi(TdApi):
    ...
    def onRspQryTradingAccount(self, data, error, n, last):
        ...
        account.balance = (data['PreBalance'] - data['PreCredit'] - data['PreMortgage'] +
                           ...
                           data['Commission'])
        account.totalProfit = (account.balance - data['PreBalance'])
        # 推送
    ...
    def onRtnOrder(self, data):
        ...
        order.leftVolume = order.totalVolume - order.tradedVolume
