...
class DataEngine(object):
    ...
    def processTickEvent(self, event):
        ...
        detail = self.getPositionDetail(tick.vtSymbol)
        detail.updateTick(tick)
    #----------------------------------------------------------------------
...
class PositionDetail(object):
    ...
    def __init__(self, vtSymbol, contract=None):
        ...
        #净多净空，持仓，持仓均价，盈利
        self.yesterdayLongNetPos = EMPTY_INT
        self.yesterdayLongPriceArray = []

        self.longNetPos = EMPTY_INT
        self.longNetPrice = EMPTY_FLOAT
        self.longNetPnl = EMPTY_FLOAT

        self.yesterdayShortNetPos = EMPTY_INT
        self.yesterdayShortPriceArray = []

        self.shortNetPos = EMPTY_INT
        self.shortNetPrice = EMPTY_FLOAT
        self.shortNetPnl = EMPTY_FLOAT
    ...
    def updatePosition(self, pos):
        ...
        if pos.direction is DIRECTION_LONG:
            #if self.longPos is 0 and self.shortPos is 0:
    ...
    def calculatePnl(self):
        ...
        self.longNetPnl = self.longNetPos * (self.lastPrice - self.longNetPrice) * self.size
        self.shortNetPnl = self.shortNetPos * (self.shortNetPrice - self.lastPrice) * self.size
    #----------------------------------------------------------------------
    def calculatePrice(self, trade):
        ...
        if trade.offset is OFFSET_OPEN:
            if trade.direction == DIRECTION_LONG:
                ...
                v = trade.volume
                while v is not 0:
                    self.ArrayLong.append(trade.price)
                    v = v - 1
            if trade.direction == DIRECTION_SHORT:
                ...
                v = trade.volume
                while v is not 0:
                    self.ArrayShort.append(trade.price)
                    v = v - 1

        elif trade.offset is OFFSET_CLOSETODAY:
            if trade.direction == DIRECTION_LONG:
                v = trade.volume
                while v is not 0 and len(self.ArrayShort):
                    self.ArrayShort.pop()
                    v = v - 1
                if len(self.ArrayShort):
                    self.shortPrice = sum(self.ArrayShort) / len(self.ArrayShort)
                else:
                    self.shortPrice = 0
            if trade.direction == DIRECTION_SHORT:
                v = trade.volume
                while v is not 0 and len(self.ArrayLong):
                    self.ArrayLong.pop()
                    v = v - 1
                if len(self.ArrayLong):
                    self.longPrice = sum(self.ArrayLong) / len(self.ArrayLong)
                else:
                    self.longPrice = 0
        #平昨计算
        elif trade.offset is OFFSET_CLOSEYESTERDAY:
            if trade.direction is DIRECTION_LONG:
                v = trade.volume
                while v is not 0:
                    self.yesterdayLongPriceArray.append(trade.price)
                    v -= 1
            elif trade.direction is DIRECTION_SHORT:
                v = trade.volume
                while v is not 0:
                    self.yesterdayShortPriceArray.append(trade.price)
                    v -= 1
        if self.longYd < 0 or self.shortYd < 0:
            #YesterdayNet = len(self.yesterdayLongPriceArray) - len(self.yesterdayShortPriceArray)
            #YesterdayNet = 0
            #time.sleep(0.06)
            pass
        YesterdayNet = self.longYd - self.shortYd

        if YesterdayNet > 0:
            self.yesterdayLongNetPos = YesterdayNet
            self.yesterdayShortNetPos = 0
            if YesterdayNet > len(self.yesterdayLongPriceArray):
                i = YesterdayNet - len(self.yesterdayLongPriceArray)
                while i != 0:
                    self.yesterdayLongPriceArray.append(self.longPrice)
                    i -= 1

        elif YesterdayNet < 0:
            self.yesterdayLongNetPos = 0
            self.yesterdayShortNetPos = 0 - YesterdayNet
            if ( 0 - YesterdayNet) > len(self.yesterdayShortPriceArray):
                i = 0 - YesterdayNet - len(self.yesterdayShortPriceArray)
                while i != 0:
                    self.yesterdayShortPriceArray.append(self.shortPrice)
                    i -= 1
        ...
        Net = len(self.ArrayLong) - len(self.ArrayShort) + YesterdayNet
        ...
        if Net > 0:
            ...
            if YesterdayNet is 0:
                self.longNetPrice = sum(
                    self.ArrayLong[(0 - Net):]) / len(self.ArrayLong[(0 - Net):])
            else:
                self.longNetPrice = sum(self.ArrayLong[(0 - Net):] + self.yesterdayLongPriceArray[(0 - YesterdayNet):]) / len(self.ArrayLong[(0 - Net):] + self.yesterdayLongPriceArray[(0 - YesterdayNet):])
            self.shortNetPrice = 0

        elif Net < 0:
            self.longNetPos = 0
            self.shortNetPos = (0 - Net)

            self.longNetPrice = 0
            if YesterdayNet is 0:
                self.shortNetPrice = sum(self.ArrayShort[Net:]) / len(self.ArrayShort[Net:])
            else:
                self.shortNetPrice = sum(self.ArrayShort[Net:] + self.yesterdayShortPriceArray[YesterdayNet:]) / len(self.ArrayShort[Net:] + self.yesterdayShortPriceArray[YesterdayNet:])
